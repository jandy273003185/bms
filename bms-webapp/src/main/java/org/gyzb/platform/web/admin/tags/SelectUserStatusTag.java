package org.gyzb.platform.web.admin.tags;
...
import org.gyzb.platform.web.admin.user.type.UserStatus;
	...
	public void doTag() throws JspException, IOException {
		...
		for (UserStatus status : UserStatus.values()) {
			if (status.name().equals(defaultValue)) {
				jspWriter.write("<option selected='selected' value='" + status.name() + "'>(" + status.name() + ")-" + status.getDesc() + "</option>");
			} else {
				jspWriter.write("<option value='" + status.name() + "'>(" + status.name() + ")-" + status.getDesc() + "</option>");
			}
		}
