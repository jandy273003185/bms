package org.gyzb.platform.web.admin.utils;
...
import org.gyzb.platform.web.returnlink.RequestLink;
import org.gyzb.platform.web.returnlink.ReturnLink;
...
public class WebUtils {
	...
	public static void init(HttpServletRequest request, HttpServletResponse response) {
		...
		LinkedList<RequestLink> requestHistory = (LinkedList<RequestLink>) httpSession.getAttribute(Constants.Platform.REQUEST_HISTORY);

		// 请求映射(增强按照名称来返回页面的业务逻辑)
		Map<String, RequestLink> requestMap = (Map<String, RequestLink>) httpSession.getAttribute(Constants.Platform.REQUEST_MAP);
		...
		if (request.getParameter(Constants.Platform.AJAX) == null && request.getParameter(Constants.Platform.MODAL_DIALOG) == null) {
    		// 如果是脚本获取,也不保存请求信息
	...
	public static void setReturnLink(String linkId) {
		...
		Map<String, RequestLink> requestMap = (Map<String, RequestLink>) getHttpSession().getAttribute(Constants.Platform.REQUEST_MAP);
	...
	public static void checkSubmitToken(){
		...
		if (sessionToken == null) {
			 ...
			 */
			LinkedList<RequestLink> links = (LinkedList<RequestLink>) sessions.get().getAttribute(Constants.Platform.REQUEST_HISTORY);
			if(!links.isEmpty()){
				links.remove(0);
			}
