...
import com.sevenpay.bms.task.service.RedPacketExpriedRefundService;
import com.sevenpay.bms.task.service.RedPacketExpriedRefundTread;
 ...
 * 
 */
@WebServlet(name = "RedPacketExpriedRefundServlet", urlPatterns = { TaskPath.BASE + TaskPath.REDPACKET_EXPRIED_REFUND })
public class RedPacketExpriedRefundServlet extends ISchedulerServlet {
	...
	private RedPacketExpriedRefundService redPacketExpriedRefundService = SpringUtils.getBean(RedPacketExpriedRefundService.class);
	...
	public String Subject(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			List<RedEnvelopeInfo> expriedInfoList = redPacketExpriedRefundService.selectExpriedRedPacket();

			logger.info("红包过期待退款的订单数量：" + expriedInfoList.size());
			if (expriedInfoList.size() > 0) {
				for (RedEnvelopeInfo expriedInfo : expriedInfoList) {
					RedPacketExpriedRefundTread redPacketExpriedRefundTread = (RedPacketExpriedRefundTread) SpringUtils
							.getBean("redPacketExpriedRefundTread");
